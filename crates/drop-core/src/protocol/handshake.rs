...
///
/// Wire format: [device_id:16][msg_count:1][version:1][msg_ids:16×N]
...
impl Handshake {
    pub const HEADER_SIZE: usize = 18; // device_id(16) + msg_count(1) + version(1)

    pub fn new(device_id: DeviceId, pending_msg_ids: Vec<Uuid>) -> Self {
        Self {
            device_id,
    ...
    pub fn from_bytes(data: &[u8]) -> Option<Self> {
        ...
        let msg_count = data[16] as usize;
        let version = data[17];
        ...
        Some(Self { device_id, version, pending_msg_ids })
    }
}
...
mod tests {
    ...
    fn test_handshake_roundtrip_empty() {
        ...
        let hs = Handshake::new(*id.device_id(), vec![]);
    ...
    fn test_handshake_roundtrip_with_messages() {
        ...
        let hs = Handshake::new(*id.device_id(), msg_ids.clone());
